...
#include <private/fa/PrFILE2/standard/pf_fread.h>

// pf2_fread - 0x815E5464, 40 bytes
// from ogws (Wii Sports VF, CC0)

pf_u32 pf2_fread(void* p_buf, pf_u32 size, pf_u32 count, struct PF_FILE* p_file) {
    pf_u32 count_read;
    PFFILE_fread(p_buf, size, count, p_file, &count_read);
    return count_read;
}
