...
#include <private/fa/PrFILE2/fatfs/pf_devinf.h>
...
pf_s32 PFVOL_InitModule(pf_u32 config, void* param);
pf_s32 PFVOL_CheckForRead(PF_VOLUME* p_vol);
pf_s32 PFVOL_CheckForWrite(PF_VOLUME* p_vol);
pf_s32 PFVOL_CheckCurrentDir(PF_VOLUME* p_vol, pf_u32 start_cluster);
pf_s32 PFVOL_GetCurrentDir(PF_VOLUME* p_vol, PF_DIR_ENT* p_current_dir);
void PFVOL_SetCurrentVolume(PF_VOLUME* p_vol);
PF_VOLUME* PFVOL_GetCurrentVolume();
pf_s32 PFVOL_SetCurrentDir(PF_VOLUME* p_vol, PF_DIR_ENT* p_current_dir);
PF_VOLUME* PFVOL_GetVolumeFromDrvChar(pf_s8 drv_char);
void PFVOL_LoadVolumeLabelFromBuf(const pf_u8* buf, PF_VOLUME* p_vol);
pf_s32 PFVOL_errnum();
pf_s32 PFVOL_getdev(pf_s8 drv_char, PF_DEV_INF* dev_inf);
pf_s32 PFVOL_attach(PF_DRV_TBL* p_drv);
pf_s32 PFVOL_detach(pf_s8 drv_char);
pf_s32 PFVOL_format(pf_s8 drv_char, const pf_u8* param);
pf_s32 PFVOL_unmount(pf_s8 drv_char, pf_u32 mode);
pf_s32 PFVOL_sync(pf_s8 drv_char, pf_u32 mode);
