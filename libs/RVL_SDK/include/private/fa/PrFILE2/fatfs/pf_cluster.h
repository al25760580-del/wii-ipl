#ifndef PRFILE2_VFMOD_PF_CLUSTER_H
#define PRFILE2_VFMOD_PF_CLUSTER_H

#include <private/fa/PrFILE2/fatfs/pf_volume.h>
#include <private/fa/PrFILE2/pf_types.h>

void PFCLUSTER_InitLastAccessCluster(PF_FILE* p_file);
void PFCLUSTER_UpdateLastAccessCluster(PF_FILE* p_file, pf_u32 sector);
void PFCLUSTER_SetLastAccessCluster(PF_FILE* p_file);
pf_s32 PFCLUSTER_AppendCluster(PF_FILE* p_file, pf_u32 byte, pf_bool* p_success, pf_u32* sector);
pf_s32 PFCLUSTER_AdjustCluster(PF_FILE* p_file);
pf_s32 PFCLUSTER_GetAppendSize(PF_FILE* p_file, pf_u32* p_size);

#endif  // PRFILE2_VFMOD_PF_CLUSTER_H
