...
#include <private/fa/PrFILE2/fatfs/pf_fat.h>
...
void PFENT_SetDotEntry(pf_u8* entry);
void PFENT_SetDotDotEntry(pf_u8* entry);
...
pf_s32 PFENT_LoadLFNEntryFieldsFromBuf(PF_DIR_ENT* p_ent, const pf_u8* buf, pf_bool is_reverse);
void PFENT_storeLFNEntryFieldsToBuf(pf_u8* buf, PF_DIR_ENT* p_ent, pf_u8 ord, pf_u8 sum, pf_bool is_last);
pf_s32 PFENT_GetEntryOfPath(PF_DIR_ENT* p_ent, PF_VOLUME* p_vol, PF_STR* p_path);
pf_s32 PFENT_GetParentEntryOfPath(PF_DIR_ENT* p_ent, PF_VOLUME* p_vol, PF_STR* p_path);
pf_s32 PFENT_findEntry(PF_FFD* p_ffd, PF_DIR_ENT* p_ent, pf_u32 index_search_from, PF_STR* p_pattern, pf_u8 attr_required, pf_u32* p_lpos,
                       pf_u32* p_ppos);
pf_s32 PFENT_allocateEntry(PF_DIR_ENT* p_ent, pf_u8 num_entries, PF_FFD* p_ffd, pf_u32* p_prev_chain, PF_STR* p_filename, pf_u8 attr_required,
                           pf_u32* p_pos);
pf_s32 PFENT_GetRootDir(PF_VOLUME* p_vol, PF_DIR_ENT* p_ent);
pf_s32 PFENT_MakeRootDir(PF_VOLUME* p_vol);
pf_u32 PFENT_CompareAttr(pf_u8 attr, pf_u32 attr_required);
pf_s32 PFENT_compareEntryName(PF_DIR_ENT* p_ent, PF_STR* p_pattern, pf_u8 attr);
pf_s32 PFENT_AdjustSFN(PF_DIR_ENT* p_ent, pf_s8* p_short_name);
pf_s32 PFENT_RemoveEntry(PF_DIR_ENT* p_ent, PF_ENT_ITER* p_iter);
pf_u8 PFENT_getcurrentDateTimeForEnt(pf_u16* p_date, pf_u16* p_time);
pf_s32 PFENT_UpdateSFNEntry(PF_DIR_ENT* p_ent, pf_bool flag);

pf_s32 PFENT_UpdateEntry(PF_DIR_ENT* p_ent, pf_u32* p_prev_chain, pf_bool is_set_ARCH);

pf_s32 PFENT_InitENT(PF_DIR_ENT* p_ent, PF_STR* p_filename, pf_u8 attr, pf_u32 is_set_time, PF_DIR_ENT* p_dir_ent, PF_VOLUME* p_vol);
pf_s32 PFENT_FillVoidEntryToSectors(PF_VOLUME* p_vol, pf_u32 start_sector, pf_u32 num_sectors, pf_u32 is_make_new_directory, PF_DIR_ENT* p_ent,
